...
func (uc *UserClient) PostVerify(mobile string, deviceID string) error {
	url := "https://app.moutai519.com.cn/xhr/front/user/register/vcode"
	body := map[string]string{
		"mobile":    mobile,
		"md5":       lib.Signature(mobile),
	...
	if imaotaiResp.Code != constant.CODESUCCESS {
		return errors.New(fmt.Sprintf("PostVerify fail,mobile:%s,%s,code:%d", mobile, imaotaiResp.Message, imaotaiResp.Code))
	}
